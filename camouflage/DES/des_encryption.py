...
def str_to_bin(string_input):
    ...
    for char in string_input:
        bin_char = format(ord(char), "08b")  # Convert character to 8-bit binary
        bin_output += bin_char
    return bin_output
...
def substring(input_str, position, length):
    return input_str[position - 1 : position + length - 1]
...
# Function to generate 16 round keys and store them in a list
...
def des_encryption(plain_text, round_keys):
    ...
    for i in range(16):
        ...
        xored = xor(right_expanded, round_keys[i])
    ...
    for index in inverse_permutation:
        ciphertext += combined_text[index - 1]
